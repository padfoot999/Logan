...
function collect {
    ...
    netstat -anf inet > $LOC/'netstat.txt'

    netstat -rn > $LOC/'netstat-routingtable.txt'

    arp -an > $LOC/'arptable.txt'
    ...
    $LSOF -i -P -n > $LOC/'lsof-netfiles.txt'
    # list parent id of open files
    $LSOF -R > $LOC/'lsof-parentid.txt'

    # system profiler
    system_profiler -xml -detaillevel full > $LOC/'sys_prof.spx'
